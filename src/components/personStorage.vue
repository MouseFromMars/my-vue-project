    ...
    return {
      ...
      persons: [
        { 
          ...
          hasActed: false, 
          attackPower: 20, 
          image: "https://cdn.culture.ru/images/e682f2a7-6f0e-574c-8a89-9569e024fe18" 
        },
        ...
        { 
          ...
          hasActed: false, 
          attackPower: 10,
          image: "https://i.pinimg.com/736x/0f/a5/0f/0fa50fee70b915298ce5b8cc743b5752.jpg"  
        },
      ],
...
selectTarget(target) {
    if (this.currentTurn === "player") {
        const activePerson = this.persons.find(person => person.hasActed === false); // Получаем активного персонажа
        if (target === "boss") {
            this.boss.hp -= activePerson.attackPower; // Урон по боссу
        } else {
            const targetPerson = this.persons.find(person => person.name === target);
            if (targetPerson) {
                targetPerson.hp -= activePerson.attackPower; // Урон по персонажу
            }
        }
        activePerson.hasActed = true;
        this.isSelectingTarget = false;

        // Проверяем, все ли персонажи сходили
